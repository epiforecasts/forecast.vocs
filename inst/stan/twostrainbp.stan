...
transformed parameters {
  ...
  vector[t_segf] mean_delta_cases;
  ...
  delta_r = tail(r, t_segf - 1) + delta_mod;
  delta_r[2:(t_segf-1)] = tail(delta_r, t_segf - 1) +
                             cumulative_sum(delta_noise * delta_eta);
  
  // non-delta growth rate based on overall with RW residuals
  r[(t_nseq+2):(t-1)] = tail(r, t_segf - 1) +
  ...
  mean_delta_cases = rep_vector(init_cases[2], t_segf);

  // log cases combined with growth
  mean_ndelta_cases[2:t] = mean_ndelta_cases[2:t] + cumulative_sum(r);
  mean_delta_cases[2:t_segf] = mean_delta_cases[2:t_segf] +
  ...
  mean_case[(t_segf + 1):t] = mean_case[(t_segf + 1):t] + mean_delta_cases;

  // rescale observation model
  phi = 1 ./ sqrt(sqrt_phi);
  
  // calculate fraction delta
  frac_delta = mean_delta_cases ./ mean_cases[(t_segf + 1):t];
}
