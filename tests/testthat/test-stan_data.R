...
test_that("Passes internal data checks with example data", {
  expect_error(stan_data(obs), NA)
})

# make default data
nlist <- stan_data(obs, horizon = h)
...
test_that("Variant strain relationship can be set as expected", {
  expect_equal(stan_data(obs, variant_relationship = "scaled")$relat, 0)
  expect_equal(stan_data(obs, variant_relationship = "pooled")$relat, 1)
  expect_equal(stan_data(obs, variant_relationship = "independent")$relat, 2)
})
