...
\begin{align}
  \lambda_t &\sim \text{LogNormal}\left(\log C_t , 0.025 \times \log C_t \right),\ t \lt P \\
  \lambda_t &= \text{exp}\left(r_t\right) \sum_{p = 1}^{P} \alpha_p  \lambda_{t-p},\ t \ge P  
\end{align}
...
\begin{align}
  \lambda_t &\sim \text{LogNormal}\left(\log C_t , 0.025 \times \log C_t \right) ,\ t \lt P\\
  \lambda^{\delta}_t &\sim \text{LogNormal}\left(\log C^{\delta}_t , 0.025 \times \log C^{\delta}_t \right),\ t \lt P \\
  \lambda^{o}_t &= \lambda_t - \lambda^{\delta}_t,\ t \lt P \\
  \lambda^{\delta}_t &= \text{exp}\left(r^{\delta}_t\right) \sum_{p = 1}^{P} \alpha^{\delta}_p  \lambda^{\delta}_{t-p},\ t \ge P \\
  \lambda^{o}_t &= \text{exp}\left(r^{o}_t\right) \sum_{p = 1}^{P} \alpha^{o}_p  \lambda^{o}_{t-p}, t \ge P   \\
...
\begin{align}
  r^{o}_0 &\sim \text{Normal}\left(0, 0.25 \right),\\
  r^{\delta}_{t_{seq}} &= r^{o}_0 + s^{\delta} \\
